...
def generate_metrics(haz_dict, ent_dict, unc_func_dist_dict={}, groups=[], risk_fncs_dict=RISK_FNCS_DICT, n_samples=1, imp_time_depend=1.2, future_year=FUTURE_YEAR, current_year=CURRENT_YEAR, file_output = None):
    ...
    for ent_key, haz_key, aggr_set in itertools.product(ent_dict.keys(), haz_dict.keys(), aggr_sets):
        ...
        for index, aggr_combo in aggr_combos_df.iterrows():
            ...
            haz_today = haz_dict[haz_key]['today'].select(extent=(ent_today.exposures.longitude.min(),
                                                                  ent_today.exposures.longitude.max(), 
                                                                  ent_today.exposures.latitude.min(), 
                                                                  ent_today.exposures.latitude.max()))
            haz_fut = haz_dict[haz_key]['future'].select(extent=(ent_fut.exposures.longitude.min(), ent_fut.exposures.longitude.max(), ent_fut.exposures.latitude.min(), ent_fut.exposures.latitude.max()))
