...
RISK_FNCS_DICT = {"aai": risk_aai_agg, "rp250": risk_rp_250}
...
def generate_metrics(
    ...
    metrics_df = pd.DataFrame(
        columns=base_col
    )  # Decision matrix (contains the criteria values we will use to base a decision)

    for ent_key, haz_key, aggr_set in itertools.product(ent_dict.keys(), haz_dict.keys(), aggr_sets):
        # Clear the output
        # clear_output()
        LOGGER.info(ent_key, haz_key, aggr_set)
        ...
        for index, aggr_combo in aggr_combos_df.iterrows():
            ...
            haz_today = haz_dict[haz_key]["today"].select(
                extent=(
                    ent_today.exposures.longitude.min(),
                    ent_today.exposures.longitude.max(),
                    ent_today.exposures.latitude.min(),
                    ent_today.exposures.latitude.max(),
                )
            )
            haz_fut = haz_dict[haz_key]["future"].select(
                extent=(
                    ent_fut.exposures.longitude.min(),
                    ent_fut.exposures.longitude.max(),
                    ent_fut.exposures.latitude.min(),
                    ent_fut.exposures.latitude.max(),
                )
            )

            if not haz_today or not haz_fut:
                LOGGER.info(f"No hazard data for {ent_key} {haz_key} {aggr_set}")
            ...
            if not unc_func_dist_dict:
                ...
                t_dict_to_df = {
                    "entity": [ent_key for meas in measure_names],
                ...
                if aggr_set:
                    t_dict_to_df.update(
                        {col_name: [col_value for meas in measure_names] for col_name, col_value in aggr_combo.items()}
                    )
                for group in groups:
                ...
                # Calc criteria – Costs
                t_dict_to_df["crit_cost"] = [
                    measure_dict[meas].cost if meas != "no measure" else 0 for meas in measure_names
                ]

                # Calc criteria –  Averted risk and cost benefit
                for name, risk_fcn in risk_fncs_dict.items():
                    LOGGER.info(haz_today)
                    LOGGER.info(haz_fut)
                    LOGGER.info(ent_today)
                    LOGGER.info(ent_fut)
                    LOGGER.info(risk_fcn)
                    LOGGER.info(imp_time_depend)
                    costbenefit_disc = CostBenefit()
                    costbenefit_disc.calc(
                        hazard=haz_today,
                        entity=ent_today,
                        haz_future=haz_fut,
                        ent_future=ent_fut,
                    ...
                    t_dict_to_df["crit_ben_" + name] = [
                        costbenefit_disc.benefit[meas] if meas != "no measure" else 0 for meas in measure_names
                    ]
                    t_dict_to_df["crit_bcr_" + name] = [
                        1 / costbenefit_disc.cost_ben_ratio[meas] if meas != "no measure" else 0
                        for meas in measure_names
                    ]
                    t_dict_to_df["crit_npv_" + name] = [
                        costbenefit_disc.tot_climate_risk - costbenefit_disc.benefit[meas]
                        if meas != "no measure"
                        else costbenefit_disc.tot_climate_risk
                        for meas in measure_names
                    ]
                ...
                # Entity today
                base_dict = {
                    "ent_today_base": ent_today,
                    "ent_fut_base": ent_fut,
                    "haz_today_base": haz_today,
                    "haz_fut_base": haz_fut,
                }
                ...
                for key in keys:
                    if unc_func_dist_dict[key]["func"] and unc_func_dist_dict[key]["distr"]:
                        iv_dict[key + "_iv"] = InputVar(
                            partial(unc_func_dist_dict[key]["func"], **base_dict), unc_func_dist_dict[key]["distr"]
                        )
                    else:
                ...
                unc_cb = CalcCostBenefit(
                    haz_input_var=iv_dict["haz_today_iv"],
                    ent_input_var=iv_dict["ent_today_iv"],
                    haz_fut_input_var=iv_dict["haz_fut_iv"],
                    ent_fut_input_var=iv_dict["ent_fut_iv"],
                )

                # Make samples (only first iteration)
                if first_iteration:
                    df_samples = unc_cb.make_sample(
                        N=n_samples, sampling_kwargs={"calc_second_order": False}
                    ).get_samples_df()
                    # Add the mean
                ...
                for name, risk_fcn in risk_fncs_dict.items():
                    ...
                    output_cb = unc_cb.uncertainty(
                        UncOutput(df_samples),
                        risk_func=risk_fcn,
                        imp_time_depen=imp_time_depend,
                        future_year=future_year,
                    )
                    df_results = output_cb.get_uncertainty(
                        metric_list=["benefit", "cost_ben_ratio", "tot_climate_risk"]
                    )

                    # Make basic empty data frame containing
                    t_crit_df = pd.DataFrame(
                        columns=keys + ["crit_ben_" + name] + ["crit_bcr_" + name] + ["crit_npv_" + name]
                    )
                    ...
                    for meas in measure_names:
                        if meas == "no measure":
                            ...
                            t_meas_crit_df["crit_npv_" + name] = (
                                df_results["tot_climate_risk"] - df_results[meas + " Benef"]
                            )
                            t_crit_df = pd.concat([t_crit_df, t_meas_crit_df], ignore_index=True)
                ...
                # Calc criteria – Costs
                t_pop_df["crit_cost"] = [
                    measure_dict[meas].cost if meas != "no measure" else 0 for meas in t_pop_df.measure
                ]
    ...
    metrics_df = metrics_df.pivot(index=base_col, columns=piv_col, values=val_col)
