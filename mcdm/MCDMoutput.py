...
class RanksOutput:
    def __init__(
        self, ranks_df, ranks_crit_df, ranks_MCDM_df, alt_exc_nan_df, alt_exc_const_df, mcdm_cols, comp_rank_cols, dm
    ):
        ...
        if isinstance(self.dm.unc_smpls_df, pd.DataFrame):
            self.counts_rank_df, self.rel_counts_rank_df = calculate_counts(
                self.dm.crit_cols, mcdm_cols, comp_rank_cols, self.dm.unc_smpls_df, ranks_df
            )
        else:
    ...
    def print_rankings(self, disp_filt={}, disp_rnk_cols=[], rank_type="MCDM", sort_by_col=None):
        ...
        for _, group_scen_df in filt_rank_df[["Group ID", "Sample ID"]].drop_duplicates().iterrows():
            ...
            if (
                ...
                LOGGER.info(f"Group: {group_id}, State: {scen_id}")
                LOGGER.info("-----------------------------------")
            ...
            LOGGER.info(tabulate(print_df, headers="keys", tablefmt="psql"))
            LOGGER.info("\n")
    ...
    def plot_rank_distribution(self, disp_rnk_col, alt_name_col="Alternative ID", sort_by_perf=True):
        ...
        for bar in ax.containers:
            for rect in bar:
                ...
                if height > 0:
                    ax.text(
                        x + width / 2, y + height / 2, str(label), ha="center", va="center", color="white", fontsize=12
